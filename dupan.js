...
! function () {
    ...
    function saveFile(i, try_flag) {
        ...
        file.md5s = file.md5s.toUpperCase();
        if (fix_dl) {
            file.md5 = randomStringTransform(file.md5);
            fix_mode = true;
        }
        $.ajax({
            url: `/api/rapidupload${check_mode||fix_mode ? '?rtype=3' : ''}`,
            type: 'POST',
            data: {
                path: dir + file.path,
                'content-md5': try_flag ? file.md5.toLowerCase() : file.md5,
                'slice-md5': try_flag ? file.md5s.toLowerCase() : file.md5s,
    ...
    const showUpdateInfo = () => {
        if (!GM_getValue('1.6.1_no_first')) {
            Swal.fire({
                title: `秒传链接提取 1.6.1 更新内容(21.3.29):`,
                html: update_info,
                heightAuto: false,
                ...
                GM_setValue('1.6.1_no_first', true);
            });
        }
    };
